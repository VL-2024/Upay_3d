...
function clampActivePiecesToField(){
  ...
  for(const p of activePieces()){
    ...
    const z = Math.max(-maxZ, Math.min(maxZ, p.position.z));
    if(Math.abs(x-p.position.x)<0.001 && Math.abs(z-p.position.z)<0.001) continue;
    p.position.x=x; p.position.z=z;
    const body=p.metadata?.aggregate?.body;
    if(body){
      try{
        const q=p.rotationQuaternion?p.rotationQuaternion.clone():BABYLON.Quaternion.FromEulerAngles(p.rotation.x,p.rotation.y,p.rotation.z);
        body.setLinearVelocity(BABYLON.Vector3.Zero());
        body.setAngularVelocity(BABYLON.Vector3.Zero());
        body.setMotionType(BABYLON.PhysicsMotionType.ANIMATED);
        body.setTargetTransform(p.position.clone(),q);
      }catch(_){}
    }
  }
}
...
function refreshBoard(){clampActivePiecesToField();freezeActivePieces();updateAllOrientations(activePieces());labels.refresh(activePieces(),store.debug&&CONFIG.debug.labels);selector.clearSelection();selector.updateVisuals(scatter.pieces);updatePairCount()}
function showScenario(){const s=scenario.snapshot();const hint=document.getElementById("hint");if(s.khanActive){const khan=khanPiece();if(khan?.metadata?.shell?.material)khan.metadata.shell.material.emissiveColor=new BABYLON.Color3(.72,.38,.02);hint.textContent=s.scenario==="TWO_KHAN"?"ЕЩЁ ОДИН ХОД! Выберите любой чүкө — цель ХАН загорится золотым.":"ХАН активирован! Выберите любой чүкө и ударьте по Хану.";return}hint.textContent=`DEMO ${s.scenario} • собрано ${s.collected}/${s.normalLimit}. Выберите подходящую пару.`}
function finishScenario(){clampActivePiecesToField();freezeActivePieces();selector.clearSelection();selector.updateVisuals(scatter.pieces);const s=scenario.snapshot();if(s.khanHit){const khan=khanPiece();if(khan?.metadata?.shell?.material)khan.metadata.shell.material.emissiveColor=new BABYLON.Color3(.95,.62,.08)}store.setState(GameState.RESULT);document.getElementById("hint").textContent=`${scenario.resultText()} • Нажмите «РАССЫПАТЬ» для следующего сценария.`}

new InputController(scene,canvas,store,selector,()=>activePieces(),(source,target)=>{if(!source||!target||!scenario.canPlay())return;const khanMove=!!target.metadata?.isKhan;if(khanMove&&!scenario.khanActive())return;if(!khanMove&&!scenario.canCollectNormal())return;if(source.metadata){source.metadata.lockedPosition=null;source.metadata.lockedQuaternion=null}selector.clearSelection();selector.updateVisuals(scatter.pieces);labels.refresh(scatter.pieces,false);store.setState(GameState.FLICKING);document.getElementById("hint").textContent=`${source.metadata.id} → ${target.metadata.id}`;const moved=flickToTarget(scene,source,target,ok=>{if(!ok){refreshBoard();store.setState(GameState.READY);showScenario();return}freezeActivePieces();if(khanMove){scenario.registerKhanHit();refreshBoard();finishScenario();return}document.getElementById("hint").textContent=`${target.metadata.id} взят. Переносим в УПАЙ…`;const collecting=collector.collect(target,result=>{const state=scenario.registerCollection();refreshBoard();if(state.finished){finishScenario();return}store.setState(GameState.READY);if(state.khanActive){showScenario();return}const progress=result?.total??state.collected;document.getElementById("hint").textContent=`DEMO ${state.scenario} • собрано ${progress}/${state.normalLimit}. Выберите следующую пару.`});if(!collecting){refreshBoard();store.setState(GameState.READY);showScenario()}});if(!moved){refreshBoard();store.setState(GameState.READY);showScenario()}},()=>scenario.khanActive());
