...
import { cloneChukoVisual } from "./chuko-model.js";
...
export function createChuko(scene,index,isKhan=false,spawn=null){
  ...
  if(isKhan){
    mat.albedoColor=new BABYLON.Color3(.92,.63,.13);
    mat.metallic=.78;mat.roughness=.24;
    mat.emissiveColor=new BABYLON.Color3(.035,.018,0);
  }else{
    const c=COLORS[index%COLORS.length];
    mat.albedoColor=new BABYLON.Color3(...c);
    mat.metallic=.04;mat.roughness=.32;
  }

  const shell=cloneChukoVisual(`visual_${index}_${isKhan?"k":"n"}`,mesh,mat,scale);
  if(!shell)throw new Error("Chuko model was not preloaded");
